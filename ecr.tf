...
resource "null_resource" "languagetool_push_to_ecr" {
  ...
  provisioner "local-exec" {
    command = <<-EOC
      ...
      docker tag ${var.languagetool_docker_image}:${var.languagetool_docker_image_tag} ${aws_ecr_repository.languagetool.repository_url}:${var.languagetool_docker_image_tag}
